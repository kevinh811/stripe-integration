...
router.post('/payment_intents', async (req, res, next) => {
  ...
  try {
    ...
    log.info('Payment amount: ', amount, ' - Intent status: Success - accepted at ', new Date().toJSON());
    return res.status(200).json({paymentIntent});
  } catch (err) {
    log.info('Payment amount: ', amount, ' - Intent status: Failure - accepted at ', new Date().toJSON());
